...
describe("authoritative Phase 6 match state", () => {
  it("processes purchases atomically and idempotently", () => {
    ...
    const first = state.execute(
      "p1",
      { type: "purchaseWeapon", requestId: "r1", weaponId: "pistol-01" },
      1200,
    );
    ...
    expect(state.players.get("p1")!.ownedWeapons).toEqual([
      "ar-01",
      "pistol-01",
    ]);
  });
  it("never charges for gates until their map effect exists", () => {
    const state = new Phase6MatchState("phase6-production", ["p1"]);
    state.award("p1", 500, 0);
    state.openShop(1000, 10000);
    const blocked = state.execute(
      "p1",
      { type: "unlockGate", requestId: "g1", gateId: "gate-east" },
      1200,
    );
    expect(blocked.ok).toBe(false);
    state.completeWave(3);
    const opened = state.execute(
      "p1",
      { type: "unlockGate", requestId: "g2", gateId: "gate-east" },
      1200,
    );
    expect(opened.reason).toBe("unavailable");
    expect(state.gates.get("gate-east")!.unlocked).toBe(false);
    expect(state.players.get("p1")!.scrap).toBe(580);
  });
