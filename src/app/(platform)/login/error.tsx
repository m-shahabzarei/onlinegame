...
export default function LoginError({ retry }: { readonly retry: () => void }) {
  ...
  return (
    <section className="mx-auto w-full max-w-2xl px-4 py-10 sm:px-6 sm:py-14">
      <ErrorState
        titleAs="h1"
        title={t("pages.signInIsUnavailable")}
        description={t("pages.weCouldNotLoadTheSigninFormRetryThe")}
        action={
          <div className="flex flex-wrap justify-center gap-3">
            <Button onClick={retry}>{t("pages.retrySignIn")}</Button>
            <Link
              className={buttonVariants({ variant: "outline" })}
              href="/continue-as-guest"
            >
              {t("pages.continueAsGuest")}
            </Link>
          </div>
        }
      />
